...
export function PropertyForm({ onSubmit, isLoading, initialData, error }: PropertyFormProps) {
  ...
  const addCustomPaymentOption = () => {
    const trimmed = newPaymentOption.trim()
    if (trimmed && !paymentOptions.includes(trimmed)) {
      setPaymentOptions((prev) => [...prev, trimmed])
      setNewPaymentOption("")
    }
  }

  const removePaymentOption = (option: string) => {
    setPaymentOptions((prev) => prev.filter((o) => o !== option))
  }
  ...
  const renderViolations = () => {
    if (typeof error === "string" || !error?.isLagosViolation || !error.violations) return null
    return (
      <Alert variant="destructive" className="mb-4">
        <AlertDescription>
          <p className="font-semibold">{error.message}</p>
    ...
    <Card>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {error && typeof error !== "string" && !error.isLagosViolation && (
            <Alert variant="destructive">
              <AlertDescription>{error.message || "Failed to save property"}</AlertDescription>
            </Alert>
          )}
            ...
            <div className="space-y-3">
              ...
              <div className="flex space-x-2">
                <Input
                  ...
                  value={newPaymentOption}
                  onChange={(e) => setNewPaymentOption(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addCustomPaymentOption())}
                />
                <Button type="button" onClick={addCustomPaymentOption} size="sm">
                  <Plus className="h-4 w-4" />
                </Button>
...
/** Small helper for labeled inputs */
...
function InputGroup({ id, label, ...props }: InputGroupProps) {
  return (
    <div className="space-y-2">
