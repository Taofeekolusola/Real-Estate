...
// import { Search, MessageSquare, Clock, AlertTriangle, CheckCircle, User, Trash2 } from "lucide-react"
...
export default function AdminDisputes() {
  ...
  const handleSelfAssign = async (dispute: IDispute) => {
    if (!user || dispute.status !== "pending") return
  ...
  const canResolveDispute = (dispute: IDispute) => {
    if (!user) return false
    if (user.role === "admin") return true
    if (dispute.assignedTo && dispute.assignedTo._id === user._id) return true
    return false
                ...
                <div className="space-y-4">
                  {filteredDisputes.map((dispute) => (
                    <div key={dispute._id} className="p-4 border rounded-lg hover:bg-gray-50">
                      ...
                      {dispute.assignedTo && (
                        <p className="text-sm text-blue-600 mb-2">
                          <strong>Assigned to:</strong> {dispute.assignedTo?.name}
                        </p>
                      )}
