...
//                     <p className="font-semibold">{formatCurrency(property.agencyFee)}</p>
//                   </div>
//                 </div>
//               </div>
...
//                   <p className="text-gray-600">Legal Fee: {formatCurrency(property.legalFee)}</p>
//                 </div>
//               )}
//             </div>
//           </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          {property.images?.length ? (
            property.images.map((image, index) => (
              <div key={index} className="relative h-64 bg-gray-200 rounded-lg overflow-hidden">
                <img src={image || "/placeholder.svg"} alt={`${property.title} - Image ${index + 1}`} className="w-full h-full object-cover" />
              </div>
            ))
          ) : (
            <div className="h-64 bg-gray-200 rounded-lg flex items-center justify-center text-gray-400">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
              ...
              <div className="mb-6">
                <h3 className="text-lg font-semibold mb-2">Payment Options</h3>
                <div className="flex flex-wrap gap-2">
                  {Array.isArray(property.paymentOptions)
                    ? property.paymentOptions.map((option, i) => <Badge key={i} variant="outline">{option}</Badge>)
                    : property.paymentOptions && <Badge variant="outline">{property.paymentOptions}</Badge>}
                </div>
              </div>
          ...
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                {property.landlord ? (
                  <div className="space-y-3">
                    <p className="font-semibold">{property.landlord.name}</p>
                    <div className="flex items-center text-gray-600"><Mail className="h-4 w-4 mr-2" /><span className="text-sm">{property.landlord.email}</span></div>
                    <div className="flex items-center text-gray-600"><Phone className="h-4 w-4 mr-2" /><span className="text-sm">{property.landlord.phone}</span></div>
                  </div>
                ) : (
                  <div className="text-center text-gray-500">
                    <p className="font-semibold">Landlord Information</p>
            ...
            {/* Booking Request */}
            {user?.role === "tenant" && (property.status === "approved" || property.approved) && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center space-x-2">
                    <MessageSquare className="h-5 w-5" />
                ...
                <CardContent>
                  {bookingSuccess ? (
                    <Alert>
                      <AlertDescription>Your inspection request has been sent successfully! The landlord will contact you soon.</AlertDescription>
                    </Alert>
                  ) : (
                    <div className="space-y-4">
                      ...
                      <Button onClick={handleBookingRequest} disabled={isBooking || !bookingMessage.trim()} className="w-full">
                        {isBooking ? "Sending Request..." : "Request Inspection"}
                      </Button>
                    </div>
                  )}
